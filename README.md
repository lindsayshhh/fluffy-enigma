...
- **Route direction is corrected from heading.** A callsign→route table stores one direction, but a flight number normally works out and back, so the stored pair is reversed for roughly half of all flights. Position can't distinguish the two — a plane between two airports looks identical either way — so the aircraft's heading decides which airport it is flying toward. The correction only applies to non-stop routes, only when the aircraft is more than 25 miles from both ends, and only when the heading is unambiguous, so departure turns, approach vectors and holds don't trigger it.
